...
| Data Generator | saloniq-data-generator | -- | One-shot | Seeds 5,008 customers, 65K+ visits, and supporting entities | PostgreSQL  |
| Feature Store | saloniq-feature-store | 8001 | Persistent | Computes and serves 30 behavioral features per customer | Data Generator (completed), Redis  |
| ML Pipeline | saloniq-ml-pipeline | -- | One-shot | Trains 4 XGBoost models, evaluates, and persists artifacts | Feature Store  |
| Prediction Service | saloniq-prediction-service | 8002 | Persistent | Loads models, scores all customers, serves REST API | ML Pipeline (completed), Redis  |
| Dashboard UI | saloniq-dashboard | 3001 | Persistent | nginx-hosted SPA with reverse proxy to prediction service | Prediction Service  |
...
```
postgres  ──┐
                     ├──> data-generator (completed) ──> feature-store 
redis    ───┘                                        |
                                                              v
...
| Scope | Customer Insight Engine and Churn Prediction |
